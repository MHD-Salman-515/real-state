...
export default function OutlierScatterView({ outliers }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-white/5 p-4">
      <h3 className="text-sm font-semibold text-white">{t("Outlier Deviation View")}</h3>
      <p className="mt-1 text-xs text-white/55">{t("Visual spread by price per m² severity")}</p>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
        {outliers.length ? (
          ...
          <p className="text-sm text-white/55">{t("No outlier points to visualize.")}</p>
        )}
      </div>
    </section>
  );
