...
export default function AddMarketRecoveryEntryModal({ open, mode, initial, onClose, onSubmit }: Props) {
  ...
  const validate = (): boolean => {
    ...
    if (!form.district.trim()) next.district = t("District is required");
    if (!Number(form.area) || Number(form.area) <= 0) next.area = t("Area must be greater than 0");
    if (!Number(form.price) || Number(form.price) <= 0) next.price = t("Price must be greater than 0");
    if (!form.source.trim()) next.source = t("Source is required");
    if (!form.entryDate) next.entryDate = t("Entry date is required");
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/70 p-4">
      <div className="max-h-[92vh] w-full max-w-4xl overflow-auto rounded-2xl border border-white/10 bg-[#0a0f1a] p-5 shadow-2xl">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h3 className="text-lg font-semibold text-white">{mode === "create" ? t("Add New Market Data") : t("Edit Market Data")}</h3>
            <p className="mt-1 text-xs text-white/55">
              {t("Use this section to inject fresh market data and recover system intelligence after major market movement.")}
            </p>
          </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            <input className={fieldClass} placeholder={t("City")} value={form.city} onChange={(e) => set("city", e.target.value)} />
            {errors.city ? <p className="mt-1 text-xs text-red-300">{errors.city}</p> : null}
          </div>
          <div>
            <input className={fieldClass} placeholder={t("District")} value={form.district} onChange={(e) => set("district", e.target.value)} />
          ...
          <div>
            <input className={fieldClass} type="number" placeholder={t("Area m²")} value={form.area} onChange={(e) => set("area", e.target.value)} />
            {errors.area ? <p className="mt-1 text-xs text-red-300">{errors.area}</p> : null}
          </div>
          <div>
            <input className={fieldClass} type="number" placeholder={t("Price")} value={form.price} onChange={(e) => set("price", e.target.value)} />
            {errors.price ? <p className="mt-1 text-xs text-red-300">{errors.price}</p> : null}
          </div>

          <input className={fieldClass} type="number" placeholder={t("Rooms")} value={form.rooms} onChange={(e) => set("rooms", e.target.value)} />
          <input className={fieldClass} type="number" placeholder={t("Bathrooms")} value={form.bathrooms} onChange={(e) => set("bathrooms", e.target.value)} />

          <input className={fieldClass} type="number" placeholder={t("Floor")} value={form.floor} onChange={(e) => set("floor", e.target.value)} />
          <input className={fieldClass} placeholder={t("Source")} value={form.source} onChange={(e) => set("source", e.target.value)} />
          ...
          <div className="col-span-full rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-sm text-white/70">
            {t("Auto Price per m²:")} <span className="font-semibold text-white">{pricePerSqm ? `${pricePerSqm.toLocaleString()} SYP` : "-"}</span>
          </div>
        </div>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button onClick={submit} className="rounded-xl border border-white/20 bg-white px-4 py-2 text-sm font-semibold text-black" type="button">
            {mode === "create" ? t("Save Entry") : t("Save Changes")}
          </button>
        </div>
      </div>
    </div>
