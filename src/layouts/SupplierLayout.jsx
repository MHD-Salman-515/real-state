...
const NAV_DEFS = [
  { to: "/supplier/tasks", key: "Tasks" },
  { to: "/supplier/bills", key: "Bills" },
  { to: "/supplier/cost-link", key: "Cost Link" },
];
...
export default function SupplierLayout() {
  ...
  useEffect(() => {
    function onAdd(e) {
      ...
      const item = {
        ...
        message: d.message || d.text || t('New supplier notification'),
  ...
  const resolveHref = (n) => { const c = validateHref(n?.href, "supplier"); return c.ok ? n?.href : c.fallbackHref; };
  const handleNotificationNavigate = (n) => { const tgt = resolveHref(n); if (!tgt) return; navigate(tgt); setNotifOpen(false); };
  ...
  const Sidebar = ({ onNav }) => (
    <aside className="dashboard-sidebar flex h-full flex-col">
      <div className="border-b border-[var(--creos-border-soft)] px-4 py-5">
        <p className="text-[11px] uppercase tracking-[0.16em] text-[color:var(--creos-muted)]">{t("CREOS Supplier")}</p>
        <h2 className="mt-1 text-lg font-semibold text-[color:var(--creos-text)]">{t('Operations Panel')}</h2>
