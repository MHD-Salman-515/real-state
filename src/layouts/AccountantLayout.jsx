...
const PAGE_TITLES = {
  ...
  "/accountant/supplier-invoices": "Supplier Invoices",
...
export default function AccountantLayout() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Keep in-memory list if local storage is unavailable.
    }
  ...
  const clearNotifications = () => {
    ...
    try {
      ...
      // Keep clear action working even if storage fails.
    }
  };

  const resolveHref = (notification) => {
    const check = validateHref(notification?.href, "accountant");
        ...
        <header className="hidden">
          <div className="flex items-center justify-between gap-3 px-4 py-3 lg:px-6">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <div className="hidden max-w-[180px] truncate text-xs text-slate-200 md:block">
                {displayName}
              </div>
              <button
