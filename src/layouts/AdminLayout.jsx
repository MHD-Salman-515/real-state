...
const NAV_DEFS = [
  { to: "/admin/appointments", key: "Appointments" },
  { to: "/admin/properties", key: "Properties" },
  { to: "/admin/maintenance", key: "Maintenance" },
  { to: "/admin/users", key: "Users" },
  { to: "/admin/commissions", key: "Commissions" },
  { to: "/admin/finance", key: "Finance" },
  { to: "/admin/aging", key: "A/R Aging" },
  { to: "/admin/operations", key: "Operations" },
  { to: "/admin/market-recovery", key: "Market Recovery" },
  { to: "/admin/intelligence-monitor", key: "AI Intelligence Monitor" },
];
...
export default function AdminLayout() {
  ...
  const pageTitle = t(PAGE_TITLE_KEYS[location.pathname] || "Admin Panel");
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw === null) { setNotifications(DEFAULT_ADMIN_NOTIFICATIONS); return; }
      const parsed = JSON.parse(raw);
      setNotifications(Array.isArray(parsed) ? parsed : []);
    } catch { setNotifications(DEFAULT_ADMIN_NOTIFICATIONS); }
  }, [userStorageKey]);
  ...
  useEffect(() => {
    function onAdd(e) {
      ...
      const item = {
        ...
        text: d.message || d.text || t('New admin notification'),
        type: d.type || "info",
        title: d.title || "",
        createdAt: ts,
        href: d.href || "",
  ...
  useEffect(() => {
    ...
    function onKeyDown(e) {
      if (e.key === "Escape") { setNotifOpen(false); setUserMenuOpen(false); setBrandMenuOpen(false); }
    }
  ...
  const Sidebar = ({ onNav }) => (
    <aside className="flex h-full flex-col border-e border-white/10 bg-[#050912]/90 backdrop-blur-xl">
      <div className="border-b border-white/10 px-4 py-5">
        <p className="text-[11px] uppercase tracking-[0.16em] text-white/90">CREOS Admin</p>
          ...
          <div className="flex items-center justify-between gap-3 px-4 py-3 lg:px-6">
            <div className="flex min-w-0 items-center gap-3">
                ...
                {brandMenuOpen ? (
                  <div className="absolute left-0 top-full z-40 mt-2 w-64 rounded-2xl border border-white/10 bg-[#050912]/95 p-2 backdrop-blur-xl">
                    <div className="border-b border-white/10 px-3 py-2">
                      ...
                      <p className="text-xs uppercase tracking-wide text-slate-400">{String(user?.role || "admin")}</p>
                    </div>
              ...
              <div className="min-w-0">
                <p className="truncate text-xs uppercase tracking-[0.14em] text-white/90">Admin Workspace</p>
            ...
            <div className="flex items-center gap-2 md:gap-3">
              <div className="hidden md:block">
                <input type="search" placeholder="Search admin tools..."
