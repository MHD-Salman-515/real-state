...
const NAV_DEFS = [{ to: "/worker", key: "Dashboard" }];
const PAGE_TITLE_KEYS = { "/worker": "Worker Dashboard" };
...
export default function WorkerLayout() {
  ...
  const NAV_ITEMS = useMemo(() => NAV_DEFS.map(d => ({ to: d.to, label: t(d.key) })), [t]);
  ...
  const pageTitle = t(PAGE_TITLE_KEYS[location.pathname] || "Worker Dashboard");
  const userStorageKey = useMemo(() => {
    const k = user?.id ?? user?.email;
    return k ? `creos_notifications__${String(k)}` : null;
  }, [user?.id, user?.email]);

  useEffect(() => {
    if (!userStorageKey) { setNotifications([]); return; }
    try {
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem(userStorageKey, JSON.stringify(notifications)); } catch {}
  ...
  useEffect(() => {
    function onAdd(e) {
      ...
      const item = {
        ...
        message: d.message || d.text || t('New worker notification'),
        type: d.type || "info", title: d.title || "",
        createdAt: ts, at: d.at || new Date(ts).toISOString(),
        href: d.href || "", hrefLabel: d.hrefLabel || "", meta: d.meta,
      };
  ...
  useEffect(() => {
    ...
    function onKeyDown(e) { if (e.key === "Escape") { setNotifOpen(false); setBrandMenuOpen(false); } }
  ...
  const clearNotifications = () => {
    setNotifications([]);
    if (!userStorageKey) return;
    try { localStorage.setItem(userStorageKey, JSON.stringify([])); } catch {}
  };

  const resolveHref = (n) => { const c = validateHref(n?.href, "worker"); return c.ok ? n?.href : c.fallbackHref; };
  const handleNotificationNavigate = (n) => { const t2 = resolveHref(n); if (!t2) return; navigate(t2); setNotifOpen(false); };
  const handleLogoNavigate = () => { setBrandMenuOpen(false); navigate(getRoleLandingPath(user)); };
  ...
  const Sidebar = ({ onNav }) => (
    <aside className="dashboard-sidebar flex h-full flex-col">
      <div className="border-b border-[var(--creos-border-soft)] px-4 py-5">
        <p className="text-[11px] uppercase tracking-[0.16em] text-[color:var(--creos-muted)]">{t("CREOS Worker")}</p>
        <h2 className="mt-1 text-lg font-semibold text-[color:var(--creos-text)]">{t('Service Desk')}</h2>
      ...
      <nav className="space-y-2 p-3">
        {NAV_ITEMS.map((item) => <Item key={item.to} to={item.to} label={item.label} onClick={onNav} />)}
    ...
    <div className="dashboard-shell grid lg:grid-cols-[280px_1fr]">
          ...
          <div className="flex items-center justify-between gap-3 px-4 py-3 lg:px-6">
            <div className="flex min-w-0 items-center gap-3">
                ...
                {brandMenuOpen ? (
                  <div className="absolute start-0 top-full z-40 mt-2 w-64 rounded-2xl border border-white/10 bg-[#050912]/95 p-2 backdrop-blur-xl">
                    <div className="border-b border-white/10 px-3 py-2">
                      ...
                      <p className="text-xs uppercase tracking-wide text-slate-400">{String(user?.role || t("worker"))}</p>
                    </div>
                    ...
                    <button type="button" onClick={handleDropdownLogout}
              ...
              <div className="min-w-0">
                <p className="truncate text-xs uppercase tracking-[0.14em] text-white/90">{t("Worker Workspace")}</p>
            ...
            <div className="flex items-center gap-2 md:gap-3">
                  ...
                  <div className="absolute end-0 z-30 mt-2 w-[340px] rounded-2xl border border-white/10 bg-[#050912]/95 backdrop-blur-xl">
                    <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
                      ...
                      <div className="flex items-center gap-2">
                        {notifications.length > 0 ? (
                          <button onClick={clearNotifications} className="text-xs text-rose-300 transition hover:text-rose-200">{t('Clear')}</button>
                        ) : null}
                        <button onClick={() => setNotifOpen(false)} className="text-xs text-slate-400 transition hover:text-white/90">{t('Close')}</button>
                      </div>
                    </div>
                    <div className="max-h-80 overflow-auto">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((n) => {
                          ...
                          return (
                            <div key={n.id}
                              className={["flex gap-2 border-b border-white/5 px-4 py-2.5 text-sm text-slate-100 last:border-b-0 hover:bg-white/5", isClickable ? "cursor-pointer" : ""].join(" ")}
                              onClick={() => handleNotificationNavigate(n)}>
                              <span className="mt-1 h-2 w-2 rounded-full bg-white/10" />
                              <div className="min-w-0 flex-1">
                                <p>{n.message || n.text}</p>
                                {isClickable ? (
                                  <button type="button" onClick={(e) => { e.stopPropagation(); handleNotificationNavigate(n); }}
                                    className="mt-1 text-xs font-medium text-white/90 transition hover:text-white/90">
                                    {n.hrefLabel || t('Open')}
                                  </button>
                                ) : null}
                                {n.createdAt ? <p className="mt-0.5 text-[11px] text-slate-500">{new Date(n.createdAt).toLocaleString()}</p> : null}
                              </div>
                            </div>
                          );
                        })
              ...
              <div className="hidden max-w-[170px] truncate text-xs text-slate-200 md:block">{displayName}</div>
              <button className="rounded-xl border border-white/20 px-3 py-1.5 text-xs text-slate-100 transition hover:bg-white/10 md:text-sm" onClick={logout}>
                {t('Logout')}
              </button>
            </div>
          </div>
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute inset-y-0 start-0 w-72"><Sidebar onNav={() => setMobileOpen(false)} /></div>
        </div>
      ) : null}
    </div>
  );
