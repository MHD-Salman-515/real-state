...
export default function Expenses() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      alert(t("Failed to save expense. Please try again."));
    }
    setSubmitting(false);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
        <p className="text-xs uppercase tracking-wider text-white/40">{t("Total Expenses")}</p>
      ...
      {loading && <p className="py-10 text-center text-white/40">{t("Loading...")}</p>}
      ...
      {modalOpen && (
        ...
        >
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#050912] p-6 shadow-2xl">
            <h2 className="mb-5 text-lg font-bold text-white">{t("New Expense")}</h2>

            <div className="space-y-4">
              <Field label={t("Description *")} placeholder={t("e.g. Office supplies")}>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block text-xs text-white/50">{t("Amount (SYP) *")}</label>
                  <input
                    type="number"
                    min="0"
                    value={form.amount}
                ...
                <div>
                  <label className="mb-1 block text-xs text-white/50">{t("Date")}</label>
                  <input
                    type="date"
                    value={form.date}
              ...
              <div>
                <label className="mb-1 block text-xs text-white/50">{t("Category")}</label>
                <input
                  type="text"
                  value={form.category}
                  onChange={e => setField("category", e.target.value)}
                  placeholder={t("e.g. Maintenance")}
                  className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white outline-none focus:border-white/20"
                />
              </div>
            </div>
